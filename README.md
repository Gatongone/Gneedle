...
> [!IMPORTANT]
> This project is still in the early stages of development, that means that I may break compatibility in order to fix major bugs or include critical features.
...
> [!WARNING]
> A template is an ordinary method, and almost all one is carried. The instructions are copied with the regions that protect them, so a `try` of a template catches where it was woven, a `using` disposes there, a `lock` releases there, and a `foreach` over an enumerator that is disposable disposes it there. The branches are carried — `if`, `switch`, `goto` and the loops — and the locals of the template are remapped to the body that is woven.
>
> A construct that the compiler writes as a method of its own is carried as well. A lambda, a local function, an `async` body, and an iterator body each live in a method beside the one the template is: the body of an `async` template, and of one that yields, is the stub that starts a state machine, whose `MoveNext` holds what was written, and a lambda leaves a type of its own. What the compiler wrote is moved onto the type being woven — a copy of it is declared inside that type, with every operand that named the original naming the copy — and the bodies that the template is pointed at are woven there, so a placeholder written inside a lambda resolves where it stands. A body that nothing points at is copied and re-pointed rather than read, because a type whose method holds no instructions is a type the runtime refuses to load.
>
> The weaving refuses a body that it cannot carry, and names what it refused: a template that reads the instance it belongs to where that template is not declared in the type being woven, one that reaches a type the compiler writes at the top level of the assembly it was compiled into rather than beside the template, one that captures a value that has no form of its own, one that holds a `ValuableMember` in a local and reads it for anything other than the member it names, one that reaches a member of an instance where the member being woven belongs to none, one that reaches a member of the instance of the member being woven from a body the compiler wrote where that body holds no instance, and one that proceeds into the body that was taken over from such a body. A delegate that names a member that it does not describe — one that hands back another value than the member hands back, or an instantiation that the constraints of the member's own parameter refuse — is refused as a member that the assembly does not hold.
...
The same two are published to npm under the names above, that the Package Manager reads through a scoped registry:
...
What is installed this way is held at the version that is named until another is, that is the difference between the two ways in: a package is taken from a registry by version, and from a repository by revision.
...
Its own command line writes that registry into the manifest and takes the package in one step, that is the way its documentation recommends:
...
The injectors that an assembly declares are applied from an entry point of your own as well, without the aspect weaver: `Injections.Apply` reads them from the attributes of the assembly, applies each of them to the member it names, and hands back the image that holds the result — or, where an injector reported something, the image it was given, because a run that reported anything leaves an assembly woven in part, that no caller could tell from one that was woven whole. The assembly is given as the one that was loaded from those bytes, because how an assembly is loaded belongs to whoever loads it, and the types that an injector names are resolved against it:
...
The attributes and the reference to the weaver are taken out of the image by default, that is what leaves the woven assembly standing alone; `removesTheWeaver: false` keeps them. What an injector is, and that member each of the interfaces of one is read for, is under [Aspect](#aspect).

> [!IMPORTANT]
> An assembly that no file holds (location missing) — one that an editor has just compiled in memory, or loaded from network stream — is given to the weaving through `AssemblyLoader.LoadFromBytes`, that loads the assembly from its image and remembers the bytes, and `Remember` does the same for an assembly that the runtime loaded by another way. That image is what a type of such an assembly is resolved against, that is how a template names a type of it through [a stub](#referring-to-a-type-you-cannot-reference). An assembly that the runtime loaded out of a file is read where that file lies, and one that it loaded from bytes in another way is read out of the memory of the process, that only Windows can do.
>
> The assemblies of the weaver are answered to such an assembly as well, and to one of those alone: the weaver that its templates were compiled against is resolved to the weaver that is weaving it, rather than to a copy that would have to lie beside the image.
...
| `ValuableMember<T>`                                                                          | the value of a field or a property of a value type, that `ValuableMember` would box    |

Each placeholder is named after what it reaches, and none of them is named after a type of the framework: a placeholder that was called `Object` would be the type that a template reads wherever it writes `Object` among the usings of this library, so that a field, a parameter or a return type that names the type of the framework would name the placeholder instead. The keyword `object` is not affected by it, and neither is a template that writes `System.Object` in full.

The name that a placeholder is given is read out of the template itself, and the one instruction that the call follows is what holds it: a name is therefore one that the compiler writes there — a literal, a `nameof`, or a constant of the template — rather than one that the template computes while it runs.
...
A name that no load stands ahead of is refused where the weaving runs, rather than left as a call that would reach the placeholder, and fail, when the member that was woven ran.
...
The two forms of that call differ in what the template has to say about the signature of the body it proceeds into. `Proceed.Method<TMethod>()` names it — a delegate, that is how the template takes arguments of its own choosing, as `left * 2` above does — while `Proceed.Invoke<TResult>()` names only the type of the value that the body hands back and passes on the arguments that the template itself was given, that for a template of an around body are the arguments of the member:

```csharp
// The same advice as above, that passes the argument on as it is: no signature is written, and no delegate is needed.
public static int AddOne(int value) => Proceed.Invoke<int>() + 1;
```

A member that hands nothing back takes `Proceed.Invoke()`, and a member whose parameters are generic takes the token — `Proceed.Invoke<M_0>()` — so that a template of such a member needs no delegate of its own to name them with. What `Invoke<TResult>` names is checked against the type that the member hands back, and a template that proceeds without a body having been taken over is refused either way.
...
A template may be a lambda rather than a method, and then it may capture the variables that it is written among. An attribute is only what a driver of the library reads, and the library is driven from the build by [the aspect weaver](#aspect) or from a call of your own by `Injections.Apply`, that [the entry point above](#weaving-from-your-own-entry-point) describes. Where the weave is asked for at a call of your own, the template is written there:
...
The delegate is what the weaving is given, so what the lambda captured is read out of it while the weaving runs, and the member that is woven carries `"Hello World"` as a string of its own rather than reaching for the instance the lambda was made from. What may be captured, and what is refused, is written out under [Principle](#principle).
...
|--------------|------------------|----------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `Aspect`     | `disable/enable` | `enable` | `disable` leaves the aspect out of the project: the scan of the solution does not add the target to it, and the task returns without reading its assembly. A project that sets any other value, and one that does not set the property at all, is woven. |
| `KeepWeaver` | `true/false`     | `false`  | `true` keeps the attributes that the injectors were read from, and the reference to the weaver that they name, in the assembly. `false` takes both back out, that is what leaves the woven assembly standing alone.                                       |
...
`Aspect` is read out of the project file itself, because the scan of a solution reads the projects that it walks without building them, and a property that comes from an imported file is not in a project's own file. A group of properties that the build reads under a condition is read for the same reason whatever the condition says, so a project that sets `disable` under a condition is a project that is left out of the weaving in every configuration it is built in. The value it is read for is `disable` alone, as the letters are written: any other value, down to another case of the same word, is a project that is woven. `KeepWeaver` is matched without regard to case, and it is passed to the task by the build, so it is an ordinary property: it is set on the command line as well as in the project file.
...
The tests of the weaver are run on `net5.0` by the runtime they were built for. A machine that carries a newer one rather than the 5.0 runtime starts their host only when it is told to move forward to the runtime it holds:

```
DOTNET_ROLL_FORWARD=LatestMajor dotnet test Inject/Tests/Gneedle.Inject.Test.csproj
```

The tests read the templates back as the IL that the compiler wrote for them, and that IL that is depends on whether the build of the assembly of tests was optimized: a temporary that the optimizer folds into the use of it is no local of the body that stands before a call, and the read of it is not there to be counted. The shapes that the assertions name are the ones a build without the optimizer writes, that is what the project asks for thatever configuration the tests are built in; the shapes that a release build of a consumer holds are the ones the optimizer writes, that is what the same tests are run against a second time:

```
DOTNET_ROLL_FORWARD=LatestMajor dotnet test Inject/Tests/Gneedle.Inject.Test.csproj -p:Optimize=true
```

that of the two legs an assembly was built as is named in its metadata, and a test of the suite reads it back: a build that does not answer the flag, or one that was left behind by the other leg, is reported rather than passing as a shape that it does not hold.

What is tested of the post processor is beside it in the package of Unity rather than here, because the compilation pipeline that those tests are written against is one that only an editor has; they are run by the test runner of an editor, that [the readme of that package](Aspect/Unity/README.md#tests) describes.
