...
internal sealed partial class MethodHandler : IMethodHandler
{
    ...
    internal string ToString(int level)
    {
        ...
        text.Append(declaration).Append(".method ").Append(TheAttributesOf(Source)).Append(' ')
            .Append(Source.ReturnType.FullName).Append(' ')
            .Append(Source.DeclaringType.FullName).Append("::").Append(Source.Name)
            .Append('(').Append(string.Join(", ", Source.Parameters.Select(parameter => TheNameOf(parameter.ParameterType, parameter.Name)))).Append(')')
            .AppendLine();
