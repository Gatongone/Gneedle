...
/// <summary>
/// Tests for <see cref="Gneedle.Aspect.GneedleILPostProcessor"/>, the step of a compilation which applies the injectors
/// of an assembly to it before it is written and loaded.<para/>
/// The sources of the post processor are compiled into this assembly, which is the name of the assembly which the
/// editor compiles them into, and the types of Unity which they are written against are the stubs which this project
/// carries, so the tests are run where no editor is installed.
/// </summary>
[TestFixture]
public class GneedleILPostProcessorTests
{
    ...
    /// </summary>
    private static Gneedle.Aspect.GneedleILPostProcessor Processor() => new();
    ...
    public void Process_Weaves_The_Assembly_Of_The_Compilation_And_Hands_The_Symbols_Back_With_It()
    {
        ...
        var assembly = new StubCompiledAssembly("Unity.Gneedle.CodeGen", image);
    ...
    public void Process_Stops_Answering_For_The_Assemblies_Of_The_Compilation_Where_It_Ends()
    {
        ...
        try
        {
            ...
            Processor().Process(new StubCompiledAssembly("UnreadableAssembly", [1, 2, 3], [path]));
            ...
            catch (FileNotFoundException)
            {
                ...
                // any more, which is what leaves the request unanswered.
            }
    ...
    /// <summary>
    /// The image of an assembly which holds the types which <paramref name="build"/> writes into it.
    /// </summary>
    ...
    private static byte[] ImageOf(string assemblyName, Action<ModuleDefinition> build)
    {
        ...
        build(assembly.Source.MainModule);

        using var written = new MemoryStream();
        assembly.SaveTo(written);
    ...
    private sealed class StubCompiledAssembly(string name, byte[] image, string[]? references = null) : ICompiledAssembly
    {
        /// <inheritdoc/>
        public InMemoryAssembly InMemoryAssembly { get; } = new(image, Symbols);
